...
public class DamageText : MonoBehaviour
{
	...
	public static DamageText Generate(GameObject prefab, Vector3 pos, DamageTextStyle style, object textContent)
	{
		...
		dmgText.Setup(textColor, textContent.ToString(), style);
		return dmgText;
	}

	// Default parent is world canvas.
	public static DamageText Generate(GameObject prefab, Vector3 pos, Color txtColor, DamageTextStyle style, object textContent)
	{
		Transform canvas = GameObject.FindWithTag("WorldCanvas").transform;
		...
		dmgText.Setup(txtColor, textContent.ToString(), style);
		return dmgText;
	}
	#endregion
